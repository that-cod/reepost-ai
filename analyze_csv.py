import pandas as pd
import json

# Read the CSV file
df = pd.read_csv('ALL_CLEANED_MERGED.csv')

print("=" * 80)
print("CSV DATA ANALYSIS REPORT")
print("=" * 80)

# Basic Information
print("\n[DATASET OVERVIEW]")
print("-" * 80)
print(f"Total Posts: {len(df):,}")
print(f"Total Columns: {len(df.columns)}")
print(f"Date Range: {df['posted_date_iso'].min()} to {df['posted_date_iso'].max()}")
print(f"Unique Authors: {df['author_name'].nunique():,}")

# Column Information
print("\n[SCHEMA DETAILS]")
print("-" * 80)
for col in df.columns:
    dtype = df[col].dtype
    non_null = df[col].count()
    null_count = df[col].isnull().sum()
    null_pct = (null_count / len(df)) * 100
    print(f"{col:25s} | {str(dtype):10s} | Non-Null: {non_null:6,} | Missing: {null_count:5,} ({null_pct:5.2f}%)")

# Engagement Metrics
print("\n[ENGAGEMENT STATISTICS]")
print("-" * 80)
print(f"Average Likes:            {df['likes'].mean():>10.2f}")
print(f"Median Likes:             {df['likes'].median():>10.2f}")
print(f"Max Likes:                {df['likes'].max():>10,.0f}")
print(f"\nAverage Comments:         {df['comments'].mean():>10.2f}")
print(f"Median Comments:          {df['comments'].median():>10.2f}")
print(f"Max Comments:             {df['comments'].max():>10,.0f}")
print(f"\nAverage Reposts:          {df['reposts'].mean():>10.2f}")
print(f"Median Reposts:           {df['reposts'].median():>10.2f}")
print(f"Max Reposts:              {df['reposts'].max():>10,.0f}")
print(f"\nAverage Engagement Score: {df['engagement_score'].mean():>10.2f}")
print(f"Median Engagement Score:  {df['engagement_score'].median():>10.2f}")
print(f"Max Engagement Score:     {df['engagement_score'].max():>10,.0f}")

# Content Metrics
print("\n[CONTENT STATISTICS]")
print("-" * 80)
print(f"Average Word Count:       {df['word_count'].mean():>10.2f}")
print(f"Median Word Count:        {df['word_count'].median():>10.2f}")
print(f"Max Word Count:           {df['word_count'].max():>10,.0f}")
print(f"Min Word Count:           {df['word_count'].min():>10,.0f}")
print(f"\nPosts with Questions:     {df['has_question'].sum():>10,} ({(df['has_question'].sum()/len(df)*100):5.2f}%)")
print(f"Posts with CTA:           {df['has_cta'].sum():>10,} ({(df['has_cta'].sum()/len(df)*100):5.2f}%)")
print(f"Posts with Hashtags:      {(df['hashtags'].notna()).sum():>10,} ({((df['hashtags'].notna()).sum()/len(df)*100):5.2f}%)")

# Post Types
print("\n[POST TYPE DISTRIBUTION]")
print("-" * 80)
post_types = df['post_type'].value_counts()
for post_type, count in post_types.items():
    pct = (count / len(df)) * 100
    print(f"{post_type:20s}: {count:6,} ({pct:5.2f}%)")

# Media Types
print("\n[MEDIA TYPE DISTRIBUTION]")
print("-" * 80)
media_types = df['media_type'].value_counts(dropna=False)
for media_type, count in media_types.items():
    pct = (count / len(df)) * 100
    media_str = str(media_type) if pd.notna(media_type) else "None/Text Only"
    print(f"{media_str:20s}: {count:6,} ({pct:5.2f}%)")

# Day of Week Distribution
print("\n[POSTING SCHEDULE - DAY OF WEEK]")
print("-" * 80)
day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
day_counts = df['posted_day_of_week'].value_counts()
for day in day_order:
    if day in day_counts.index:
        count = day_counts[day]
        pct = (count / len(df)) * 100
        print(f"{day:15s}: {count:6,} ({pct:5.2f}%)")

# Hour Distribution
print("\n[POSTING SCHEDULE - HOUR OF DAY]")
print("-" * 80)
hour_counts = df['posted_hour'].value_counts().sort_index()
for hour, count in hour_counts.items():
    pct = (count / len(df)) * 100
    print(f"{hour:2d}:00 | {count:5,} ({pct:5.2f}%)")


# Top Authors
print("\n[TOP 10 AUTHORS BY POST COUNT]")
print("-" * 80)
top_authors = df['author_name'].value_counts().head(10)
for idx, (author, count) in enumerate(top_authors.items(), 1):
    print(f"{idx:2d}. {author:40s}: {count:4,} posts")

# Engagement by Post Type
print("\n[AVERAGE ENGAGEMENT BY POST TYPE]")
print("-" * 80)
engagement_by_type = df.groupby('post_type').agg({
    'likes': 'mean',
    'comments': 'mean',
    'reposts': 'mean',
    'engagement_score': 'mean'
}).round(2)
print(engagement_by_type)

# Engagement by Media Type
print("\n[AVERAGE ENGAGEMENT BY MEDIA TYPE]")
print("-" * 80)
engagement_by_media = df.groupby('media_type').agg({
    'likes': 'mean',
    'comments': 'mean',
    'reposts': 'mean',
    'engagement_score': 'mean'
}).round(2)
print(engagement_by_media)

# Hashtag Analysis
print("\n[HASHTAG INSIGHTS]")
print("-" * 80)
posts_with_hashtags = df[df['hashtags'].notna()]
if len(posts_with_hashtags) > 0:
    # Count hashtags per post
    hashtag_counts = posts_with_hashtags['hashtags'].str.split(',').str.len()
    print(f"Average hashtags per post (when used): {hashtag_counts.mean():.2f}")
    print(f"Max hashtags in a single post:         {hashtag_counts.max()}")
    
# Key Insights
print("\n[KEY INSIGHTS]")
print("-" * 80)

# Best performing day
best_day = df.groupby('posted_day_of_week')['engagement_score'].mean().idxmax()
print(f">> Best day to post: {best_day}")

# Best performing hour
best_hour = df.groupby('posted_hour')['engagement_score'].mean().idxmax()
print(f">> Best hour to post: {best_hour}:00")

# Posts with questions vs without
if df['has_question'].sum() > 0:
    with_q = df[df['has_question'] == True]['engagement_score'].mean()
    without_q = df[df['has_question'] == False]['engagement_score'].mean()
    diff_pct = ((with_q - without_q) / without_q) * 100
    print(f">> Posts with questions get {diff_pct:+.2f}% more engagement")

# Posts with CTA vs without
if df['has_cta'].sum() > 0:
    with_cta = df[df['has_cta'] == True]['engagement_score'].mean()
    without_cta = df[df['has_cta'] == False]['engagement_score'].mean()
    diff_pct = ((with_cta - without_cta) / without_cta) * 100
    print(f">> Posts with CTA get {diff_pct:+.2f}% more engagement")

# Optimal word count
word_count_engagement = df.groupby(pd.cut(df['word_count'], bins=[0, 50, 100, 150, 200, 300, 500, 1000]))['engagement_score'].mean()
best_range = word_count_engagement.idxmax()
print(f">> Best word count range: {best_range}")

print("\n" + "=" * 80)
print("END OF REPORT")
print("=" * 80)
